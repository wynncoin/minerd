...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params)
{
	...
	//@wynn
    state = json_string_value(json_array_get(params, 8));
    utxo = json_string_value(json_array_get(params, 9));
	//
	...
	if (!job_id || !prevhash || !coinb1 || !coinb2 || !version || !nbits || !ntime ||
	    ...
	    strlen(nbits) != 8 || strlen(ntime) != 8 || strlen(state) != 64 || strlen(utxo) != 64) {
	...
	//@wynn
	hex2bin(sctx->job.hashStateRoot,state,32);
	hex2bin(sctx->job.hashUTXORoot,utxo,32);
	...
	hex2bin(sctx->job.version, version, 4);  //20000000
