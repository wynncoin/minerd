...
enum algos {
    ALGO_SHA256TX,     /* SHA-256tx */
};

static const char *algo_names[] = {
    [ALGO_SHA256TX] =     "sha256tx",
};
...
static enum algos opt_algo = ALGO_SHA256TX;
...
static bool submit_upstream_work(CURL *curl, struct work *work) {
    ...
    if (!submit_old && memcmp(work->data + 1, g_work.data + 1, 32)) {
        if (opt_debug)
            applog(LOG_DEBUG, "DEBUG: stale work detected, discarding %08x %08x ",work->data[1] ,g_work.data[1]);
        return true;
    }
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work) {
    ...
    if (jsonrpc_2) {
        ...
        sha256tx(merkle_root, sctx->job.coinbase, sctx->job.coinbase_size);
        for (i = 0; i < sctx->job.merkle_count; i++) {
            memcpy(merkle_root + 32, sctx->job.merkle[i], 32);
            sha256d(merkle_root, merkle_root, 64);
        }
#ifdef DDDDD
	    printf(" ./cli decoderawtransaction ");
	    for( int i = 0 ; i < sctx->job.coinbase_size ; i++){
		    printf("%02x",sctx->job.coinbase[i]);
	    }
	    printf("\n");
        ...
        */
        ...
        //让出一个 nonce
        for (i = 0; i < 8; i++){
            work->data[20 + i] = be32dec((uint32_t *) sctx->job.hashStateRoot + 7-i);
        }
        for (i = 0; i < 8; i++){
            work->data[28 + i] = be32dec((uint32_t *) sctx->job.hashUTXORoot +  7-i);
        }

        //prevoutStake
        for (i = 0; i < 8; i++)work->data[36 + i] = 0;work->data[44] = 0xffffffff ;
        //work->data[45] = 0x80000000;    //20       
        //work->data[46] = 0x00000280;    //21
#ifdef DDDDD        
				printf("%s.%d>>full blockheader:",__func__,__LINE__);
				unsigned char * p = (unsigned char * )work->data;
				for( int i = 0 ; i < 180 ; i ++ ){
					printf("%02x",p[i]);
				}
				printf("\n");
...
static void *miner_thread(void *userdata) {
    ...
    while (1) {
...
#ifdef DDDDD            
            printf("miner thread  copy work prevhash %08x %08x\n",work.data[1],g_work.data[1]);
#endif            
        ...
        rc = scanhash_sha256tx(thr_id, work.data, work.target, max_nonce,
                    &hashes_done);



#ifdef DDDDD
        printf("scanhash prevhash %08x %08x\n",work.data[1],g_work.data[1]);
#endif
