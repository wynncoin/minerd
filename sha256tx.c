...
void sha256tx(unsigned char *hash, const unsigned char *data, int len){
    ...
    sha256( hash1 , data  , len );
    sha256( hash2 , hash1 , 32  );
    sha256( hash  , hash2 , 32  );
    for( int i = 0 ; i < 32; i++ ){
        hash[i] ^= hash1[i];
    }
}

void print_hash( unsigned char * s){
	for( int i = 0 ; i < 32 ; i++){
		printf("%02x",s[i]);
	}
}
...
#ifdef DDDDD
void dump_header( unsigned char * p){
	printf("version   : %08x\n",*(unsigned int*)p);p+=4;
	printf("prevblock : ");  print_hash(p); printf("\n");p+=32;
	printf("merkleroot: " );  print_hash(p); printf("\n");p+=32;
	printf("ntime: %08x \n" , *(unsigned int*)p);p+=4;
	printf("nbits: %08x \n" , *(unsigned int*)p);p+=4;
	printf("nonce: %08x \n" , *(unsigned int*)p);p+=4;
...
int scanhash_sha256tx(int thr_id, uint32_t *pdata, const uint32_t *ptarget,uint32_t max_nonce, uint64_t *hashes_done){
	...
	data[45] = 0 ; //proof-stake block signature null
#ifdef DDDDD	
	dump_header(data);
#endif    
	do {
        data[19] = ++n;
		sha256tx((unsigned char *)hash, (unsigned char *)data,181);
